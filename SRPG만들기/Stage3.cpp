...
CStage3::CStage3(CFrameWork * framework, HWND hWnd, CurrentScene tag)
	:CScene(framework, hWnd)
{
	INDEX idx;
	m_map = new CMap(_T("resource/map/성벽.txt"));
	m_map->LoadUnit(_T("resource/map/성벽_유닛.txt"), &m_vecUnits);
	...
	m_vecUnits[3]->SetUnitWay(UnitWay::RightDown);
	m_vecUnits[5]->SetUnitWay(UnitWay::RightDown);
	m_vecUnits[8]->SetUnitWay(UnitWay::LeftUp);
	m_vecUnits[10]->SetUnitWay(UnitWay::LeftUp);
	m_vecUnits[12]->SetUnitWay(UnitWay::LeftUp);
	m_vecUnits[14]->SetUnitWay(UnitWay::LeftUp);
	m_pFrameWork->GetSound()->AddStream(("resource/sound/BGM/STAGE3_BGM.mp3"), Sound_Name::BGM_STAGE3);
	...
	camera.viewPoint.y = 30;

	m_bCommandSelecting = TRUE;
	CUI_ClearCondition::GetInstance()->MakeUI();
	CUI_ClearCondition::GetInstance()->SetPhase(CScene::CurrentScene::Stage3);
}
CStage3::~CStage3()
{
	delete m_map;
...
void CStage3::OnUpdate(float timeElapsed)
{
...
void CStage3::DrawIntervalScene(HDC hdc)
{
	//static int count = 0;
	...
	else
	{
		if (CheckGameWin())
		{
			...
			//CProfile::GetInstance()->AddStageClear(3);
			//CheckAchievement();
...
BOOL CStage3::CheckGameWin()
{
	for (int i = 0; i < (int)m_vecUnits.size(); ++i)
	{
		if (m_vecUnits[i]->GetUnitName() == UnitName::Ivan
			&&m_vecUnits[i]->GetHp() == 0)
			return TRUE;
	}
	return FALSE;
}
void CStage3::PlaySound()
{
	m_pFrameWork->GetSound()->PlayBgm(Sound_Name::BGM_STAGE3);
}
BOOL CStage3::OnProcessingKeyboardMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (wParam)
	{
	case 'C':
...
BOOL CStage3::OnProcessingMouseMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (iMsg)
	{
	case WM_LBUTTONDOWN:
