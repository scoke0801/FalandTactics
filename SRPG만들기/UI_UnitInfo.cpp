...
CUI_UnitInfo* CUI_UnitInfo::instance = NULL;
CUI_UnitInfo::CUI_UnitInfo(HWND hWnd, HINSTANCE hInstance, CUnit* unit)
	:CUI(hWnd, hInstance)
{
	szWindowClass = _T("UnitInfo_Class");
...
void CUI_UnitInfo::OnUpdate()
{
	ShowUI();
	InvalidateRect(hUi,&m_rcRect, FALSE);
}
void CUI_UnitInfo::RegisterUI()
{
	if (!m_bRegistration)
	{
		WNDCLASSEXW wcex;
...
BOOL CUI_UnitInfo::CompareUnit(CUnit* unit)
{
	if (m_pUnit == unit)
		return TRUE;
	return FALSE;
}
void CUI_UnitInfo::DrawUnit(HDC hdc)
{
	if(m_pUnit == NULL)
		return;
...
void CUI_UnitInfo::ReDraw()
{
	InvalidateRect(hUi, NULL, FALSE);
}
CUI_UnitInfo * CUI_UnitInfo::GetInstance(HWND hWnd, HINSTANCE hInstance, CUnit* unit)
{
	if (instance == NULL)
	{
		instance = new CUI_UnitInfo(hWnd, hInstance, unit);
	}
	return instance;
}
CUI_UnitInfo * CUI_UnitInfo::GetInstance()
{
	return instance;
}

LRESULT CALLBACK UI_UnitInfo_Proc(HWND hWnd, UINT iMsg,
	WPARAM wParam, LPARAM lParam)
{	
	HDC hdc, bufferDC; //화면에 출력할 hdc, 출력하기 전까지 저장할 변수 bufferDC
	PAINTSTRUCT ps;
	static RECT rect;
	static CUI_UnitInfo* self;
	static HBITMAP		hBmp;
