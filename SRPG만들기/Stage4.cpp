...
CStage4::CStage4(CFrameWork * framework, HWND hWnd, CurrentScene tag)
	:CScene(framework, hWnd)
{
	INDEX idx;
	m_map = new CMap(_T("resource/map/제단.txt"));
	m_map->LoadUnit(_T("resource/map/제단_유닛.txt"), &m_vecUnits);
	...
	for (int i = 0; i < 11; ++i)
	{
		if (i % 2 == 0)
			...
			m_vecUnits[i]->SetUnitWay(UnitWay::RightDown);
	}
	for (int i = 11; i < 19; ++i)
	{
		if(i % 2 == 0)
			m_vecUnits[i]->SetUnitWay(UnitWay::LeftUp);
		else
			m_vecUnits[i]->SetUnitWay(UnitWay::RightUp);
	}
	m_vecUnits[2]->SetUnitWay(UnitWay::RightUp);
	m_vecUnits[1]->SetUnitWay(UnitWay::RightUp);
	m_vecUnits[0]->SetUnitWay(UnitWay::LeftUp);
	m_vecUnits[12]->SetUnitWay(UnitWay::RightUp);
	m_vecUnits[13]->SetUnitWay(UnitWay::LeftUp);
	m_pFrameWork->GetSound()->AddStream(("resource/sound/BGM/STAGE4_BGM.mp3"), Sound_Name::BGM_STAGE4);
	...
	CUI_ClearCondition::GetInstance()->SetPhase(CScene::CurrentScene::Stage4);
...
void CStage4::OnUpdate(float timeElapsed)
{
...
void CStage4::OnDraw(HDC hdc)
{
	...
	CSystemImage::GetInstance()->DrawPortal(hdc, m_map->GetTilePos(13, 0), camera);
...
void CStage4::DrawIntervalScene(HDC hdc)
{
	//static int count = 0;	
	...
	else
	{
		if (CheckGameWin())
		{
			...
			//CProfile::GetInstance()->AddStageClear(4);
			//CheckAchievement();
...
void CStage4::PlaySound()
{
	m_pFrameWork->GetSound()->PlayBgm(Sound_Name::BGM_STAGE4);
}

BOOL CStage4::CheckGameWin()
{
	INDEX idx;
	//INDEX compIdx;
	...
	for (int i = 0; i < (int)m_vecUnits.size(); ++i)
	{
		...
		{
			idx = m_vecUnits[i]->GetCurrentIndex();
			/*compIdx = m_vecUnits[i]->GetNextTileIndex();

			if (compIdx.x != 14
				|| compIdx.y != 1)
				return FALSE;
*/
			if (idx.x == 14 && idx.y == 1 && !m_vecUnits[i]->GetCanMove())	
				return TRUE;
...
BOOL CStage4::OnProcessingKeyboardMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (wParam)
	{
	case 'C':
...
BOOL CStage4::OnProcessingMouseMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (iMsg)
	{
	case WM_LBUTTONDOWN:
