...
CStage2::CStage2(CFrameWork * framework, HWND hWnd, CurrentScene tag)
	:CScene(framework, hWnd)
{
	m_map = new CMap(_T("resource/map/고원.txt"));
	m_map->LoadUnit(_T("resource/map/고원_유닛.txt"), &m_vecUnits);
	...
	m_pFrameWork->GetSound()->AddStream(("resource/sound/BGM/STAGE2_BGM.mp3"), Sound_Name::BGM_STAGE2);
	...
	camera.viewPoint.y = 250;

	m_bCommandSelecting = TRUE;
	CUI_ClearCondition::GetInstance()->SetPhase(CScene::CurrentScene::Stage2);
...
void CStage2::OnUpdate(float timeElapsed)
{
...
void CStage2::DrawIntervalScene(HDC hdc)
{
	//static int count = 0;
	...
	else
	{
		if (CheckGameWin())
		{
			...
			//CProfile::GetInstance()->AddStageClear(2);
			//CheckAchievement();
...
void CStage2::PlaySound()
{
	m_pFrameWork->GetSound()->PlayBgm(Sound_Name::BGM_STAGE2);
}
BOOL CStage2::CheckGameWin()
{
	for (int i = 0; i < (int)m_vecUnits.size(); ++i)
	{
		if (m_vecUnits[i]->GetUnitName() == UnitName::Catherine
			&&m_vecUnits[i]->GetHp() == 0)
			return TRUE;
	}
	return FALSE;
}
BOOL CStage2::OnProcessingKeyboardMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (wParam)
	{
	case 'C':
...
BOOL CStage2::OnProcessingMouseMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (iMsg)
	{
	case WM_LBUTTONDOWN:
