...
CStageUser::CStageUser(CFrameWork * framework, HWND hWnd, CurrentScene tag, LPCTSTR szMap, LPCTSTR szUnit)
	:CScene(framework, hWnd)
{
	INDEX idx;
	m_map = new CMap(szMap);
	m_map->LoadUnit(szUnit, &m_vecUnits);
	...
	m_pFrameWork->GetSound()->AddStream(("resource/sound/BGM/STAGE_USER_BGM.mp3"), Sound_Name::BGM_STAGE_USER);
...
CStageUser::~CStageUser()
{
	delete m_map;
}

void CStageUser::OnUpdate(float timeElapsed)
{
...
void CStageUser::OnDraw(HDC hdc)
{
	static int delay = 0;
...
void CStageUser::DrawIntervalScene(HDC hdc)
{
	//static int count = 0;
...
void CStageUser::PlaySound()
{
	m_pFrameWork->GetSound()->PlayBgm(Sound_Name::BGM_STAGE_USER);
}
BOOL CStageUser::OnProcessingKeyboardMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (wParam)
	{
	case 'C':
...
BOOL CStageUser::OnProcessingMouseMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (iMsg)
	{
	case WM_LBUTTONDOWN:
