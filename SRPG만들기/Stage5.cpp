...
CStage5::CStage5(CFrameWork * framework, HWND hWnd, CurrentScene tag)
	:CScene(framework, hWnd)
{
	INDEX idx;
	m_map = new CMap(_T("resource/map/다리.txt"));
	m_map->LoadUnit(_T("resource/map/다리_유닛.txt"), &m_vecUnits);
	...
	m_vecUnits[6]->SetUnitWay(UnitWay::RightDown);
	m_pFrameWork->GetSound()->AddStream(("resource/sound/BGM/STAGE5_BGM.mp3"), Sound_Name::BGM_STAGE5);
	...
	CUI_ClearCondition::GetInstance(
		m_pFrameWork->GetHWND()
		, m_pFrameWork->GetHINSTANCE()
		, CScene::CurrentScene::Stage5
		);
	CUI_UnitDetailInfo::GetInstance(
		m_pFrameWork->GetHWND()
		, m_pFrameWork->GetHINSTANCE()
		);
	camera.viewPoint.x = 150;
	camera.viewPoint.y = 150;

	m_bCommandSelecting = TRUE;
	CUI_ClearCondition::GetInstance()->SetPhase(CScene::CurrentScene::Stage5);
...
void CStage5::OnUpdate(float timeElapsed)
{
...
void CStage5::DrawIntervalScene(HDC hdc)
{
	//static int count = 0;
	...
	else
	{
		if (CheckGameWin())
		{
			...
			//CProfile::GetInstance()->AddStageClear(5);
			//CheckAchievement();
...
void CStage5::PlaySound()
{
	m_pFrameWork->GetSound()->PlayBgm(Sound_Name::BGM_STAGE5);
}
BOOL CStage5::OnProcessingKeyboardMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (wParam)
	{
	case 'C':
...
BOOL CStage5::OnProcessingMouseMessage(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch (iMsg)
	{
	case WM_LBUTTONDOWN:
